		   ...
		   sector_t isect, sector_t len)
{
	...
	list_for_each_entry(be, &bl->bl_extents, be_node) {
		...
		if (isect < be->be_f_offset + be->be_length) {
			if (be->be_state == INVALID_DATA) {
				inval = be;
				break;
			} else if (be->be_state == READ_WRITE_DATA ||
				   be->be_state == NEEDS_INIT)
				rv = be;
		}
	}
	if (!inval) {
...
{
	if (be->be_state == READ_WRITE_DATA)
		return 1;
	else if (be->be_state != NEEDS_INIT)
		return 0;
	else {
		...
		struct nfs_write_data *wdata)
{
	...
	dprintk("%s submitting write bio\n", __FUNCTION__);
...
map_block(sector_t isect, struct pnfs_block_extent *be,
	  struct block_device *dev, struct buffer_head *res_bh,
	       ...
	       unsigned count, struct pnfs_fsdata **fsdata)
{
	...
	*fsdata = bl_use_pnfs;
	...
	dprintk("%s inval=%i, from=%u, to=%u\n", __FUNCTION__, inval, from, to);
	if (inval) {
		if (be->be_state == NONE_DATA) {
			dprintk("%s PANIC - got NONE_DATA extent %p\n",
				__FUNCTION__, be);
			*fsdata = bl_use_mds;
			goto cleanup;
		}
		map_block(isect, be, BLK_ID(lseg->layout)->bm_mdev,
			  bh, PAGE_CACHE_SHIFT - 9);
	...
	if (PageUptodate(page)) {
		...
		map_block(isect, read_extent, BLK_ID(lseg->layout)->bm_mdev,
			  bh, PAGE_CACHE_SHIFT - 9);
		lock_buffer(bh);
	     ...
	     unsigned count, unsigned copied, struct pnfs_fsdata *fsdata)
{
	...
	print_page(page);
	if (fsdata) {
		int *statusp = (int *)fsdata;
		dprintk("%s status=%i\n", __FUNCTION__, *statusp);
		if (*statusp)
...
bl_new_request(struct pnfs_layout_segment *lseg, struct nfs_page *req,
	       u64 pos, u64 count, struct pnfs_fsdata *fsdata)
{
	dprintk("%s enter %Lu@%Lu\n", __FUNCTION__, count, pos);
