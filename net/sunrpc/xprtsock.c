				       ...
				       struct xdr_skb_reader *desc)
{
	...
	if (req == NULL) {
		 ...
		 */
		dprintk("RPC:       Couldn't get rpc_rqst for the callback!\n");
		dprintk("RPC:       Schedule autoclose of the connection\n");
...
static int bc_send_request(struct rpc_task *task)
{
	...
	dprintk("sending request with xid: %u\n", req->rq_xid);
	/*
...
static struct rpc_xprt *xs_setup_tcp(struct xprt_create *args)
{
	...
	if (args->svsk) {
		 ...
		 */
		xprt->svsk = args->svsk;
		xprt->svsk->sk_xprt = xprt;
		transport->sock = xprt->svsk->sk_sock;
		transport->inet = xprt->svsk->sk_sk;
